...
/**
 * A single RSI three-entry-system ladder step. Generated by the backend's
 * `generate_trade_plan()` for both LONG (RSI 30/24/16) and SHORT
 * (RSI 80/92/95) directions.
 */
export interface RsiEntry {
  /** Human label, e.g. "Entry 1". */
  entry: string;
  /** Trigger description, e.g. "RSI hits 30". */
  trigger: string;
  /** Position size label, e.g. "20%". */
  position_size: string;
  /** Numeric RSI threshold (30 / 24 / 16 for longs; 80 / 92 / 95 for shorts). */
  rsi_target: number;
}

/**
 * The full nested trade-plan object (as opposed to the flat one). Exposes the
 * hidden DCA data from the scan response: the RSI three-entry ladder, DCA
 * strategy rules, risk-management rules, the list of met confirmations, and
 * the OTE entry zone. All fields are nullable for the no-trade case.
 */
export interface TradePlanFull {
  rsi_entry_system?: { current_rsi: number; entries: RsiEntry[] } | null;
  dca_strategy?: string[] | null;
  risk_management?: string[] | null;
  confirmations_met?: string[] | null;
  entry_zone?: { low: number; high: number; description: string } | null;
